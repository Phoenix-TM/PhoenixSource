...
do
local sudo = 53045150,92241574,166694622,95867902,216558673 --put your id here(BOT OWNER ID)
...
local function res_user_callback(extra, success, result) -- /info <username> function
   ...
   Username = 'No Username'
    ...
    if not value then
	 if result.id == tonumber(sudo) then
	   text = text..'Rank : Executive Admin (Executive Admin) \n\n'
  ...
  local uhash = 'Username:'..result.id
...
local function action_by_reply(extra, success, result)-- (reply) /info  function
  ...
  local text = 'FirstName : '..(result.from.first_name or '')..'\n LastName:'..(result.from.last_name or '----')..'\n'
               ..'Username : '..Username..'\n'
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = 'FirstName : '..(msg.from.first_name or 'Not found')..'\n'
   local text = text..'LastName : '..(msg.from.last_name or 'Not found')..'\n'	
   local text = text..'Username : '..Username..'\n'
   local text = text..'ID : '..msg.from.id..'\n\n'
   local text = text..'Phone Number : '..msg.from.phone..'\n'
   local hash = 'rank:variables'
	if hash then
